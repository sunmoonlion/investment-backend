# investment-backend 镜像构建

## 架构

- **构建上下文**：子模块根目录（`investment-backend/`）
- **源码位置**：`app/` 子目录（pyproject.toml、uv.lock、应用代码）
- **构建方式**：多阶段构建（python:slim 安装依赖 → python:slim 运行时）
- **镜像名称**：`investment-backend:1.0.0`（本地）；CI 使用 git SHA tag
...
--context       <子模块根目录>
--build-arg     REGISTRY=harbor.sunmoonai.com:30443/k8s-images
--destination   harbor.sunmoonai.com:30443/k8s-images/investment-backend:<git-sha>
--destination   harbor.sunmoonai.com:30443/k8s-images/investment-backend:latest
```
