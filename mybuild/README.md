...
- **镜像名称**：`investment-backend:architecture-v2-dev`（本地）；CI 使用 git SHA tag
...
docker build -f mybuild/Dockerfile \
  ...
  -t investment-backend:architecture-v2-dev .
```
...
--context       <子模块根目录>
--build-arg     REGISTRY=harbor.sunmoonai.com:30443/k8s-images
--destination   harbor.sunmoonai.com:30443/app-images/investment-backend:<git-sha>
```
