...
```text
Secret:    research-admin-backend-s3
ConfigMap: research-admin-backend-s3
```
...
```bash
./k8s-scaffold/scaffold.sh research-admin-backend 8001 \
  --type backend \
  --with-object-storage
```
...
```text
RESEARCH_ADMIN_BACKEND_OBJECT_STORAGE_CONFIGMAP_NAME=research-admin-backend-s3
RESEARCH_ADMIN_BACKEND_OBJECT_STORAGE_SECRET_NAME=research-admin-backend-s3
```
