...
async def async_main(args: argparse.Namespace) -> None:
    async with httpx.AsyncClient(base_url=args.base_url, timeout=10.0) as client:
        ...
        run_response = await client.post(
            ...
            json={
                "idempotency_key": f"deployed-{uuid.uuid4()}",
        ...
        print(
            json.dumps({"step": "resume_run", "run_id": run_id}, ensure_ascii=False),
            flush=True,
        )
        resume_response = await client.post(
            ...
            json={
                ...
                "user_input": {"text": "continue"},
                "idempotency_key": f"resume-{uuid.uuid4()}",
            },
        )
        ...
        print(
            json.dumps(
                {
                    "deployed_agent_http_validation": "ok",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate deployed MoocManus v4 API + Celery worker flow."
    )
