...
def _settings() -> Settings:
    return Settings(
        ...
        casdoor_client_id="research-admin-client",
        casdoor_client_secret="test-only-secret",
        casdoor_redirect_uri="https://research.example.test/api/auth/callback",
        casdoor_application="sunmoonai-research-admin",
        frontend_base_url="https://research.example.test",
...
def _token(key: RSAKey, **overrides) -> str:
    now = int(time.time())
    claims = {
        "iss": "https://identity.example.test/.well-known/sunmoonai-research-admin",
        "sub": "user-123",
        ...
        if request.url.path.endswith("/openid-configuration"):
            return httpx.Response(
                ...
                json={
                    ...
                    "jwks_uri": "https://identity.example.test/.well-known/sunmoonai-research-admin/jwks",
                },
            )
...
async def test_backchannel_transport_preserves_public_issuer_and_host() -> None:
    ...
    assert claims["iss"] == (
        "https://identity.example.test/.well-known/sunmoonai-research-admin"
    )
    assert paths == [
        "/.well-known/sunmoonai-research-admin/openid-configuration",
        "/api/login/oauth/access_token",
        "/.well-known/sunmoonai-research-admin/jwks",
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"aud": ["research-admin-client", "info-admin-client"]},
        {"nonce": "wrong-nonce"},
