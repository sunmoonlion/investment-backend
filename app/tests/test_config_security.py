...
def test_production_browser_identity_is_fail_fast() -> None:
    ...
    assert settings.session_cookie_name == "sunmoonai_research_admin_sid"
    assert settings.transaction_cookie_name == "sunmoonai_research_admin_oidc_tx"
    assert settings.session_key_prefix == "research:auth:admin:session:"
    ...
    with pytest.raises(ValueError, match="standard discovery path"):
        _production_settings(
            casdoor_discovery_url=(
                "https://identity.example.test/sunmoonai-research-admin/"
