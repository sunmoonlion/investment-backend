...
async def test_logout_is_post_only_and_csrf_protected(
    ...
    fake = FakeAuthService({"admin": _session("research:admin")})
    monkeypatch.setattr(auth_middleware, "_auth_service", fake)
    monkeypatch.setattr(auth_routes, "_auth_service", fake)
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(
...
def test_every_non_auth_api_route_requires_a_scope_dependency() -> None:
    ...
    for route in app.routes:
        ...
        # Flatten nested router deps: look at route.dependant and parent
        assert calls & allowed or any(
            getattr(d.call, "__name__", "") in allowed
            for d in route.dependant.dependencies
        ) or "dependency" in {
            getattr(d.call, "__name__", "")
            for d in (route.dependant.dependencies or [])
        }, (route.path, calls)
        # Prefer: at least one known guard appears in the dependency tree
        names=set()
        ...
        assert names & allowed, (route.path, names)
