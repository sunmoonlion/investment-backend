...
> **维护**：本目录位于 **research-admin-backend** 内，为该 backend 配套 **`db-access-bootstrap` 的唯一 `dbctl` 来源**；与独立 `k8s` 仓库无联动。将 admin 仓库单独迁出时请一并带走本目录。
...
- `REDIS_DB_INDEX`
- `REDIS_KEY_PREFIX`（默认 `${SERVICE_NAME}:*`）：**可空格分隔多个 key 模式**，例如 `"session:* research:*"`，须覆盖应用实际写入的 key（Nest BFF 会话为 `session:*`，仅 `research:*` 会导致无法 `SET session:`）。
